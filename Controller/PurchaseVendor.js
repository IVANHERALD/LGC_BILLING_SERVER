...
export const createVendor = async (req, res, next) => {
    try {
        const {vendor_id,vendor_name, vendor_gstin,vendor_contact,vendor_email,vendor_address,vendor_city,vendor_state,vendor_pin_code,  vendor_account_holder_name,vendor_account_number,vendor_bank_name,vendor_ifsc_code,vendor_branch_name } = req.body;
        if (!vendor_id || !vendor_name || !vendor_gstin || !vendor_contact|| !vendor_email||!vendor_address||!vendor_city||!vendor_state||!vendor_pin_code||  !vendor_account_holder_name||!vendor_account_number||!vendor_bank_name||!vendor_ifsc_code||!vendor_branch_name) {
            return res.status(422).json({ message: 'Invalid Inputs' });
        }
        const existingVendor = await PurchaseVendorDetails.findOne({ vendor_name });
        if (!existingVendor) {
            const VendorItem = new PurchaseVendorDetails({vendor_id,
